...
function linkLayout(options) {
  ...
  function layout(G2) {
    setEdgeEndpoints(G2, options, edgeGap);
...
function sortEdgesByConnectedNodeY(G2, edges, direction) {
  return edges.slice().sort((a, b) => {
    let ya, yb;
    if (direction === "outgoing") {
      const nodeA = G2.node(a.w);
      const nodeB = G2.node(b.w);
      ya = (nodeA == null ? void 0 : nodeA.y) ?? 0;
      yb = (nodeB == null ? void 0 : nodeB.y) ?? 0;
    } else {
      const nodeA = G2.node(a.v);
      const nodeB = G2.node(b.v);
      ya = (nodeA == null ? void 0 : nodeA.y) ?? 0;
      yb = (nodeB == null ? void 0 : nodeB.y) ?? 0;
    }
    if (Math.abs(ya - yb) > 1e-3) {
      return ya - yb;
    }
    if (a.name !== b.name) {
...
function setEdgeEndpoints(G2, options, edgeGap) {
  G2.nodes().forEach((u) => {
    const node = G2.node(u);
    const outgoingEdges = sortEdgesByConnectedNodeY(G2, node.outgoing || [], "outgoing");
    const incomingEdges = sortEdgesByConnectedNodeY(G2, node.incoming || [], "incoming");
    const outgoingCount = outgoingEdges.length;
    const incomingCount = incomingEdges.length;
    const outgoingEdgesHeight = outgoingEdges.reduce((sum, e) => sum + G2.edge(e).dy, 0);
    const incomingEdgesHeight = incomingEdges.reduce((sum, e) => sum + G2.edge(e).dy, 0);
    const outgoingGapsTotal = Math.max(0, outgoingCount - 1) * edgeGap;
    const incomingGapsTotal = Math.max(0, incomingCount - 1) * edgeGap;
    const outgoingTotalHeight = outgoingEdgesHeight + outgoingGapsTotal;
    const incomingTotalHeight = incomingEdgesHeight + incomingGapsTotal;
    let sy = (node.dy - outgoingTotalHeight) / 2;
    let ty = (node.dy - incomingTotalHeight) / 2;
    sy = Math.max(0, sy);
    ty = Math.max(0, ty);
...
function justifiedPositioning() {
  ...
  function calculateEdgeGapHeight(G2, nodeId, edgeGap) {
    if (edgeGap <= 0)
      return 0;
    const inEdges = G2.inEdges(nodeId) || [];
    const outEdges = G2.outEdges(nodeId) || [];
    const inGaps = Math.max(0, inEdges.length - 1);
    const outGaps = Math.max(0, outEdges.length - 1);
    return Math.max(inGaps, outGaps) * edgeGap;
  }
  function position(G2, order, edgeGap = 0) {
    if (scale === null)
      position.scaleToFit(G2, order, edgeGap);
    setNodeValues(G2, edgeValue, nodeDirection);
    setNodeEdgeHeights(G2, edgeValue, scale, edgeGap);
    const bandVals = bandValues(G2, order, edgeGap, scale);
    order.forEach((rank) => {
      let y = 0;
      rank.forEach((band, j) => {
  ...
  position.scaleToFit = function(G2, order, edgeGap = 0) {
    setNodeValues(G2, edgeValue, nodeDirection);
    let totalEdgeGapContribution = 0;
    if (edgeGap > 0) {
      G2.nodes().forEach((n) => {
        totalEdgeGapContribution = Math.max(totalEdgeGapContribution, calculateEdgeGapHeight(G2, n, edgeGap));
      });
    }
    const maxValue = sumBy(bandValues(G2, order, 0, 1));
    if (maxValue <= 0) {
      scale = 1;
      return;
    }
    const availableHeight = size[1] - totalEdgeGapContribution;
    scale = availableHeight / maxValue;
    if (whitespace !== 1)
      scale *= 1 - whitespace;
  };
...
function setNodeEdgeHeights(G2, edgeValue, scale, edgeGap = 0) {
  G2.nodes().forEach((n) => {
    const node = G2.node(n);
    const inEdges = G2.inEdges(n) || [];
    const outEdges = G2.outEdges(n) || [];
    const inGaps = Math.max(0, inEdges.length - 1);
    const outGaps = Math.max(0, outEdges.length - 1);
    const maxGaps = Math.max(inGaps, outGaps);
    node.dy = node.value * scale + maxGaps * edgeGap;
  });
  G2.edges().forEach((e) => {
    const edge = G2.edge(e);
...
function bandValues(G2, order, edgeGap = 0, scale = 1) {
  ...
  order.forEach((rank) => {
    rank.forEach((band, j) => {
      const total = sumBy(band, (u) => {
        const node = G2.node(u);
        const inEdges = G2.inEdges(u) || [];
        const outEdges = G2.outEdges(u) || [];
        const inGaps = Math.max(0, inEdges.length - 1);
        const outGaps = Math.max(0, outEdges.length - 1);
        const maxGaps = Math.max(inGaps, outGaps);
        return node.value + maxGaps * edgeGap / scale;
      });
...
function sankey(options) {
  ...
  let links = [];
  let order = null;
  if (options.whitespace !== void 0) {
    nodeLayout.whitespace(options.whitespace);
  }
  function layout(linksIn = [], nodesIn = [], data = {}) {
    ...
    nodes = nodeLayout(graph2, order, options.edgeGap || 0);
...
class Paper {
  ...
  exportToSvg() {
    const exportInstance = new Export(this.canvas, "apexsankey");
...
class SankeyGraph extends Paper {
  constructor(element, options, chartContext) {
    super(element, options, chartContext);
  }
  construct(data) {
    const { edgeGap, nodeWidth, spacing, viewPortHeight, viewPortWidth, whitespace } = this.options;
    const layout = sankey({ edgeGap, nodeWidth, whitespace });
  ...
  renderEdge(edgeObj, group) {
    ...
    if (enableTooltip && sourceNode.data && targetNode.data) {
      const tooltipContent = tooltipTemplate ? tooltipTemplate({ source: sourceNode.data, target: targetNode.data, value: edgeLabel.value }) : `${sourceNode.data.title} → ${targetNode.data.title}: ${edgeLabel.value}`;
      path.on("mouseover", (event) => {
        ...
        updateTooltip(this.chartContext, tooltipId, tooltipStyles, tooltipContent);
      });
      ...
      path.on("mouseout", () => {
        const tooltip = getTooltip(this.chartContext, tooltipId);
...
class ApexSankey extends BaseChart {
  constructor(element, options = {}) {
    const instanceId = "sankey_" + Math.random().toString(36).substr(2, 9);
    super(element, instanceId);
    this.options = { ...DefaultOptions, ...options };
    const { height, width } = this.options;
    this.graph = new SankeyGraph(this.element, this.options, this.chartContext);
    this.setupElementDimensions(width, height);
    this.handleWatermark();
  }
  static setLicense(key) {
    LicenseManager.setLicense(key);
  }
  setupElementDimensions(width, height) {
